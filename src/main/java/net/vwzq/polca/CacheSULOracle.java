...
public class CacheSULOracle implements MealyMembershipOracle<String, String> {
    ...
    public CacheSULOracle(CacheSUL sul, Config conf, String label, float probability) {
        this.sul = sul;
		this.config = conf;
		this.label = label;
    ...
    public void processQueries(Collection<? extends Query<String, Word<String>>> queries) {
		synchronized (sul) {
                processQueries(sul, queries, this.config, label, probability);
        }
    }

    private static void processQueries(CacheSUL sul, Collection<? extends Query<String, Word<String>>> queries, Config config, String label, float probability) {
        for (Query<String, Word<String>> q : queries) {
			if (config.verbose) System.out.println("(" + label + ") " + "processQuery: " + q);
			Word<String> output = answerQuery(sul, q.getPrefix(), q.getSuffix(), probability);
            q.answer(output);
        }
    }

    @NonNull
    public static Word<String> answerQuery(CacheSUL sul, Word<String> prefix, Word<String> suffix, float probability) {
        sul.pre();
        try {
